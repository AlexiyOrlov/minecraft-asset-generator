        ...
        generate.setOnAction(new EventHandler<ActionEvent>() {
            ...
            void handle(ActionEvent evnt) {
                ...
                if (modidentr && blockidenr) {
                    ...
                    File modelFile = Paths.get(path.text, AssetConstants.ASSETS.value, modidentr, AssetConstants.MODELS_LITERAL.value, AssetConstants.BLOCKMODEL.value, blockidenr + '.json').toFile()
                    String texturePath
                    ...
                    if (texturePath) {
                        if (modelFile.createNewFile()) {
                            ...
                            switch (parent) {
                                case BlockState.SIMPLE_BLOCK:
                                    modelContent = Utilities.formatJson([parent: BlockModels.BLOCKSINGLETEXTURE.value, textures: [all: texturePath]])
                                    break
                                case BlockState.CROSS:
                                    modelContent = Utilities.formatJson([parent: BlockModels.CROSS.value, textures: [cross: texturePath]])
                                    break
                                case BlockState.DIFFERENT_SIDES:
                                    modelContent = Utilities.formatJson([parent  : BlockModels.BLOCKDIFFERENTSIDES.value,
                                    ...
                                    break
                                case BlockState.COLUMN:
                                    modelContent = Utilities.formatJson([parent: BlockModels.BLOCKCOLUMN.value, textures: [side: texturePath,
                                                                                                                           end : texturePath + '_end']])
                                    break
                                case BlockState.TOP_BOTTOM_SIDE:
                                    modelContent = Utilities.formatJson([parent  : BlockModels.BLOCKDIFFERENTTOPANDBOTTOM.value,
                                                                         textures: [top : texturePath + '_top', bottom: texturePath + '_bottom',
                                                                                    side: texturePath]])
                                    break
                                case BlockState.DIRECTIONAL:
                                    modelContent = Utilities.formatJson([parent  : BlockModels.BLOCKDIRECTIONAL.value,
    ...
    private static String createSimpleBlockstate(MinecraftVersion version, String mod, String blockId) {
        ...
        switch (version) {
            case MinecraftVersion.V1_12:
                ...
                break
            case MinecraftVersion.V1_15:
                content = Utilities.formatJson([variants: [(""): [model: "$mod:$AssetConstants.BLOCKMODEL.value/$blockId"]]])
                break
        }
        return content
    }
